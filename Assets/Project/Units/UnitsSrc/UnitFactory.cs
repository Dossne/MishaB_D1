...
namespace TetrisTactic.Units
{
    public sealed class UnitFactory
    {
        ...
        {
            return new UnitRuntimeModel(unitConfig.CreateUnitData(UnitType.Player), position);
        }
        public UnitRuntimeModel CreateEnemy(UnitType enemyType, GridPosition position)
        {
            return new UnitRuntimeModel(unitConfig.CreateUnitData(enemyType), position);
        }
